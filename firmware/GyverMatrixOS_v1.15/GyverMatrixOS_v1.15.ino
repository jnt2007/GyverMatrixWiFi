...
byte IP_STA[] = {192, 168, 88, 205};          // Статический адрес матрицы в локальной сети WiFi

#define DEFAULT_NTP_SERVER "0.ua.pool.ntp.org" // NTP сервер по умолчанию "time.nist.gov"
...
#define PHOTO_PIN 0           // пин фоторезистора
#define LED_PIN 0            // пин ленты D3
#define PIN_BTN 2            // кнопка подключена сюда (PIN --- КНОПКА --- GND)
#define SRX D7                // не используется, но требуется для компиляции скетча
#define STX D8                // не используется, но требуется для компиляции скетча
...
#define DEMO_GAME_SPEED 80    // скорость игр в демо режиме (мс)
...
boolean showDateInClock = false;    // Показывать дату при отображении часов
...
bool overlayEnabled = false;        // Разрешено наложение часов на эффекты
...
String TEXT_1 = "LED";             // Строка заданного цвета
String TEXT_2 = "MATRIX";          // Строка с плавной сменой цвета
String TEXT_3 = "WIND";            // Строка с разноцветными буквами
String runningText = "";           // Текущий текст бегущей строки, задаваемый пользователем со смартфона
byte textColorMode = 0;            // Режим отображения цвета бегущей строки: 0 - globalColor (монохром); 1 - радуга; 2 - цветные буквы;
...
void setup() {
  ...
  */
  ...
  // Текст для демо-режимов "бегущая строка"
  TEXT_1 = String(F("Героiв Сталiнграда 13-А"));       // Строка заданного цвета
  TEXT_2 = String(F("Героiв Сталiнграда 13-А"));            // Строка с плавной сменой цвета
  TEXT_3 = String(F("Героiв Сталiнграда 13-А"));                            // Строка с разноцветными буквами
